...
import { getAllDistributors } from "../api/api";
 ...
 * - Matches Manufacturer page's modern UI/UX
 * - Includes stats, search, filtering, and refresh
...
// debounce hook for smooth search typing
...
const Distributors = () => {
  ...
  const counts = useMemo(() => {
    const all = distributors.length;
    const approved = distributors.filter((d) => d.isApproved === true).length;
    const pending = distributors.filter((d) => d.isApproved === false).length;
  ...
  const filtered = useMemo(() => {
    ...
    const byFilter = distributors.filter((d) => {
      if (filter === "all") return true;
      if (filter === "approved") return d.isApproved === true;
      if (filter === "pending") return d.isApproved === false;
      return true;
    });

    if (!q) return byFilter;

    return byFilter.filter((d) => {
      const fields = [
        d.name,
        d.email,
        d.phone,
  ...
  }, [distributors, filter, debouncedQuery]);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage distributor approvals, verify licenses and details.
          </p>
        </div>
      ...
      {/* Distributor Table */}
      <DistributorTable distributors={filtered} refresh={fetchDistributors} onRowClick={openModal} />
