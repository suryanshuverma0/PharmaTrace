...
import { getAllPharmacists } from "../api/api";
import PharmacistModal from "../components/PharmacistModal";
import PharmacistTable from "../components/PharmacistTable"; // We'll create this similar to DistributorTable
...
const Pharmacists = () => {
  ...
  const counts = useMemo(() => {
    const all = pharmacists.length;
    const approved = pharmacists.filter((p) => p.isApproved).length;
    const pending = pharmacists.filter((p) => !p.isApproved).length;
  ...
  const filtered = useMemo(() => {
    ...
    const byFilter = pharmacists.filter((p) => {
  ...
  }, [pharmacists, filter, debouncedQuery]);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage pharmacist approvals, verify licenses and details.
          </p>
        </div>
      ...
      {/* Pharmacist Table */}
      <PharmacistTable pharmacists={filtered} refresh={fetchPharmacists} onRowClick={openModal} />
