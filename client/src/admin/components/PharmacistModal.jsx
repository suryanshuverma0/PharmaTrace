...
const PharmacistModal = ({ isOpen, closeModal, pharmacist, refresh }) => {
  ...
  const handleApprove = async (approve) => {
    setLoading(true);
    try {
      const res = await approvePharmacist(pharmacist._id, approve);
      if (res?.success) {
              ...
              >
                <Dialog.Panel className="w-full max-w-5xl bg-white rounded-3xl shadow-2xl p-8 relative overflow-hidden">
                  ...
                  <div className="grid md:grid-cols-2 gap-6 mb-6">
                    <Field label="Email" value={pharmacist.email} />
                    <Field label="Phone" value={pharmacist.phone} />
                    <Field label="Country" value={pharmacist.country} />
                    <Field label="State" value={pharmacist.state} />
                    <Field label="City" value={pharmacist.city} />
                    <Field label="Website" value={pharmacist.website} />
                    <Field
                      label="Status"
                      value={
                  ...
                  <div className="grid md:grid-cols-2 gap-6 mb-6">
                    <Field
                      label="Pharmacy Name"
                      value={pharmacist.pharmacist?.pharmacyName}
                    />
                    <Field
                      label="License Number"
                      value={pharmacist.pharmacist?.licenseNumber}
                    />
                    <Field
                      label="Pharmacy Location"
                      value={pharmacist.pharmacist?.pharmacyLocation}
                    />
                    ...
                    <div className="col-span-full">
                      ...
                      {pharmacist.pharmacist?.licenseDocument ? (
                        <div className="mt-2 border rounded-xl overflow-hidden shadow-md hover:shadow-xl transition">
                          <a
                            href={pharmacist.pharmacist.licenseDocument}
                            target="_blank"
                            rel="noopener noreferrer"
                          >
                            <img
                              src={pharmacist.pharmacist.licenseDocument}
