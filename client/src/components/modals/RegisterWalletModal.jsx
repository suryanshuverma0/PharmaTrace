...
const roleFieldsConfig = {
  ...
  manufacturer: [
    ...
    { name: "licenseDocument", label: "License Document", type: "file", accept: ".pdf,.doc,.docx", required: false },
...
const RegisterWalletModal = () => {
  const { connectWallet, registerUserWithRole, isLoading, isConnected, checkAccountLoading } = useAuth();
  const { isRegisterModalOpen, closeRegisterModal, openConnectModal } = useWalletModal(); // ✅ use context

  const [formData, setFormData] = useState({
    name: "", email: "", phone: "", role: "", country: "", city: "", state: "",
    companyName: "", registrationNumber: "", licenseDocument: "", website: "",
    certifications: "", warehouseAddress: "", operationalRegions: "",
    pharmacyName: "", licenseNumber: "", pharmacyLocation: "",
  });
    ...
    try {
      ...
      roleFields.forEach((field) => {
        if (formData[field.name]) {
          roleData[field.name] = (field.name === "certifications" || field.name === "operationalRegions")
  ...
  return (
    <Modal isOpen={isRegisterModalOpen} onClose={closeRegisterModal} size="lg" className="max-w-xl">
