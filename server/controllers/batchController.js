...
const registerBatch = async (req, res) => {
  ...
  try {
    ...
    if (!batchNumber || !manufactureDate || !expiryDate || !quantityProduced || !dosageForm || !strength || !productName) {
      return res.status(400).json({ 
        message: "All required fields (batchNumber, manufactureDate, expiryDate, quantityProduced, dosageForm, strength, productName) must be provided" 
      });
    }
    ...
    const batch = new Batch({
      ...
      autoGenerateProducts: true, // Enable auto-generation by default
      productGenerationStatus: 'pending', // Set to pending for cron job processing
    });

    await batch.save();

    // Trigger immediate product generation
    if (batch.autoGenerateProducts) {
      console.log(`🚀 Starting immediate product generation for batch: ${batch.batchNumber}`);
      
      // Import and use the service directly
      const ProductAutoGeneration = require('../services/productAutoGeneration');
      
      // Start generation in background (non-blocking)
      process.nextTick(async () => {
        try {
          await ProductAutoGeneration.generateProductsForBatch(batch);
          console.log(`✅ Product generation completed for batch: ${batch.batchNumber}`);
        } catch (error) {
          console.error(`❌ Product generation failed for batch ${batch.batchNumber}:`, error.message);
        }
      });
    }

    res.status(201).json({ 
      message: batch.autoGenerateProducts 
        ? "Batch registered successfully. Product generation started immediately in the background." 
        : "Batch registered successfully",
      batch: batch.toObject(),
      autoGeneration: batch.autoGenerateProducts
    });
...
// Get batch auto-generation status
const getBatchGenerationStatus = async (req, res) => {
  try {
    const { batchId } = req.params;
    const batch = await Batch.findById(batchId)
      .select('batchNumber productGenerationStatus autoGenerateProducts quantityAvailable quantityProduced');
    
    if (!batch) {
      return res.status(404).json({ message: 'Batch not found' });
    }
    
    // Count generated products
    const generatedCount = await Product.countDocuments({ batchId: batch._id, autoGenerated: true });
    
    res.json({
      batchNumber: batch.batchNumber,
      status: batch.productGenerationStatus,
      autoGenerate: batch.autoGenerateProducts,
      quantityProduced: batch.quantityProduced,
      quantityAvailable: batch.quantityAvailable,
      productsGenerated: generatedCount,
      progress: batch.quantityProduced > 0 ? Math.round((generatedCount / batch.quantityProduced) * 100) : 0
    });
  } catch (error) {
...
const getBatchVerificationStatus = async (req, res) => {
  try {
    const { batchNumber } = req.params;
    ...
    const verificationStatus = {
      manufacturerVerified: batch.manufacturerVerified,
      distributorVerified: batch.distributorVerified,
      pharmacistVerified: batch.pharmacistVerified,
      verificationTimestamps: batch.verificationTimestamps
    };
    
    // Also get blockchain verification status if needed
    try {
      const blockchainStatus = await batchContract.getVerificationStatus(batchNumber);
      verificationStatus.blockchain = {
        manufacturerVerified: blockchainStatus[0],
        distributorVerified: blockchainStatus[1],
        pharmacistVerified: blockchainStatus[2],
        manufacturerVerifiedAt: blockchainStatus[3].toString(),
        distributorVerifiedAt: blockchainStatus[4].toString(),
        pharmacistVerifiedAt: blockchainStatus[5].toString()
      };
    } catch (blockchainError) {
      console.log('Could not fetch blockchain verification status:', blockchainError.message);
      verificationStatus.blockchain = null;
    }
    ...
    console.error('Error fetching batch verification status:', error);
    res.status(500).json({ 
      success: false,
